...
<script>
import { usersList, updateNote } from "@/api/sales";
import Users from "./components/users"; // secondary package based on el-table
...
export default {
  ...
  methods: {
      ...
      usersList(this.query).then(
        response => {
          ...
          this.list = list.map(v => {
            ...
            v.originalNote = v.sale_note; //  will be used when user click the cancel botton
            v.note = v.sale_note;
            return v;
          });
        },
    ...
    update(msg) {
      var data = { id: msg.id, sale_note: msg.note };
      ...
      error => {
        this.$message({
          message: error.response.data.sale_note[0],
          type: "error"
        });
      });
    }
