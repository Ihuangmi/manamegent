...
<script>
import { usersList, updateNote } from "@/api/market";
...
export default {
  ...
  methods: {
      ...
      usersList(this.query).then(
        response => {
          ...
          this.list = list.map(v => {
            ...
            v.originalNote = v.market_note; //  will be used when user click the cancel botton
            v.note = v.market_note;
            return v;
          });
        },
        error => {
    ...
    update(msg) {
      var data = { id: msg.id, market_note: msg.note };
